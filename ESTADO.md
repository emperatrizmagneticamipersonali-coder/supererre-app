...
⏸️ CHECKPOINT — Segunda pulida del onboarding (referencia visual del usuario, app "Keiki"): (1) el León mascota ahora hace las preguntas en un globo de conversación (componente MascotBubble) en vez de un h1 plano — nombre/edad/dolor/interés; (2) la pantalla de "Buena noticia" ahora muestra al León grande + León chico ("[nombre] y su León guía") en vez de un solo emoji; (3) la pantalla de carga ahora tiene una mini-escena ilustrada (mapa + León + sparkles) sobre el anillo de progreso; (4) LA MÁS IMPORTANTE: la primera victoria (Rugido del León) ahora usa el micrófono REAL del dispositivo (Web Audio API: getUserMedia + AnalyserNode calculando RMS) para detectar si el niño realmente rugió — ya no avanza con un simple tap. Tiene fallback honesto si el micrófono no está disponible/permitido ("Continuar sin micrófono"), nunca bloquea al usuario. Todo el audio se procesa localmente, nunca se sube a ningún servidor (coherente con la promesa de privacidad de FICHA-AVATAR.md). Probado de punta a punta: build + typecheck + eslint limpios, sin overflow a 375px, fallback de micrófono verificado en el navegador de pruebas (el micrófono real requiere el celular del usuario) / Siguiente acción exacta: proponer Sesión 5 (app interna: Praxias, Onomatopeyas/AR, Escalera Fonética, Minijuego)
